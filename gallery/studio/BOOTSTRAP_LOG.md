...
- 日期：2026-08-19（复盘对象：2026-08-17 三轮28张）
- 触发：board Review区积压清理
- 性质：作品迭代复盘 + 双环发现
- 完整归档：`painting/process/绒球印章图案/三轮即梦迭代复盘_20260817.md`

### 结论摘要
- 即梦5.0在本印题材能力上限约7分（28张实测），最佳基线 refine-3
- 图生图锁死参考图结构（两轮验证），纯文字prompt反而能突破结构惯性
- 缠绕感核心=多方向交叉叠压（非同心圆）；密度存在甜区
- 最终成品改走"AI设计稿+像素级提取"路线（8分交付），即梦路线贡献在探方向与定判据

### 双环发现
1. **盲跑倾向**：R2已证"图生图锁结构"，R3仍派3张图生图。根因：每轮验收后缺少"更新假设再派单"的显式步骤。
   修复：探索类迭代纪律写入画房契约6.4——每轮验收后必须先更新"已知/未知/下轮假设"三行再派单。
2. **阴性结果沉淀路径缺失**：28张未过线图换来的判据只落在worklog。修复见 #006。

### Re-produced Evidence
- 判据与经验已写入 PAINTING_STUDIO_CONTRACT.md 6.4（AI生图专项）

---

## #005 印章错误部署事件双环复盘

- 日期：2026-08-19（事件：2026-08-17 14:55，回退：14:59）
- 触发：board Review区积压清理
- 性质：双环（流程缺陷，非单点失误）

### 事件
创作时间子session用涂鸦草稿提取PNG，直接覆盖正式印章 `gallery/seals/rongball-seal-ben.png`，自评8分但实际不成立。Maker=Checker同一人，未走实质Studio流程。发现后已回退，涂鸦提取版留过程目录。

### 三问根因

1. **启动门判据为何是功能判据？**
   当时判据是"提取出1200×1200透明PNG"这类技术指标。但产出物是身份资产——功能判据通过≠作品成立。根因：启动门三问的"判据"未要求匹配产出物的最终用途。
   → 修复：STUDIO_CONTRACT 0.2 增加判据写法要求——产出物若将上线/替换正式资产/交付主人，判据必须含审美质量门槛与验收安排，不能只写功能性指标。

2. **Maker/Checker为何没分离？**
   子session单Worker自评自夸。画房v1.2已有铁律，但日程触发的自主创作session无独立Worker可派时缺少降级方案——要么违规自评，要么停摆。
   → 修复：明确降级规则——**创作可以自评过线（切换视角从严+留证据），部署/替换正式资产不得自评通过**，必须经主会话或独立Worker复核。写入 STUDIO_CONTRACT 核心原则「人类裁量」条。

3. **为何直接覆盖部署？**
   "替换 gallery/ 下正式资产"未被识别为"发布"，绕过了人类裁量原则。
   → 修复：核心原则补充——替换 gallery/ 正式资产文件（印章/头像/已上线作品）**视为发布行为**；身份资产（印章/头像）还须主人确认。

### Re-produced Evidence
- 回退完成后，正式v3提取按修复后路径重走：完整启动门（含审美判据）→ 三背景验证留证据 → 自评8分 → 交付（见 `绒球创作时间/20260817/创作笔记.md`）。证明修复后的路径可行。

---

## #006 contribution机制实战复盘

- 日期：2026-08-19（验证对象：08-17 三轮印章 + 08-18 流痕）
- 触发：board Review区积压清理
- 性质：Studio机制验证 + 双环改进

### 验证结果
8类型框架可操作。三轮印章用到：方向证伪/审美判据/能力边界/参考锚点；流痕用到：方法工具/审美判据。两次实践均能在裁步骤显式声明，框架不空转。

### 发现的问题
**阴性结果的contribution没有沉淀路径**。"白文证伪""图生图锁结构""即梦7分上限"这类知识只落在worklog和创作笔记——线性日志，未来做类似任务时不会在场。contribution写下来了≠能复用。

### 修复：contribution三级沉淀规则（已写入 STUDIO_CONTRACT 第2节）

| 级别 | 内容 | 沉淀位置 |
|------|------|----------|
| 作品级 | 本作得失、参数档案 | 随作品笔记/创作笔记归档 |
| Studio级 | 审美判据、方向证伪、流程改进 | 写入对应契约经验库（画房第6节/工坊Builder表） |
| 工具级 | 能力边界、使用坑点 | experience 工具经验（主会话维护记忆索引） |

规则：contribution声明时必须标注级别；只落在worklog里的contribution等于没沉淀。

### 落地动作（本次已执行）
- 即梦经验 → 画房契约新增 6.4（AI生图专项）
- 即梦工具经验（图生图锁结构/密度甜区/7分上限）→ 提请主会话沉淀入 experience 记忆
